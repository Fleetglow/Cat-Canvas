...
- FastAPI 默认监听固定地址 `127.0.0.1:47960`，端口被占用时回退到随机本地端口；单次启动令牌保护 HTTP 与 WebSocket。
...
当前稳定桌面版为 `0.1.9`：支持默认最大化、启动恢复上次画布、桌面设置持久化、API 生成参数记忆、生成按钮防连点，以及 GitHub/Gitee 双源更新、Gitee 静态分块、minisign/SHA-256 校验、更新前备份和自动重启安装。保留 `0.1.0` 作为首个桌面版基线；多画布标签页计划放在 `0.2.0`。
